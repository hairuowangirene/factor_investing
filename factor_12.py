...
class factor_12():
    ...
    def generate(self):
        ...
        for _, ukeydf in group:
            ...
            ukeydf = vwap(ukeydf)
            ukeydf = ts_sum(ukeydf, 'vwap_5', n=sum)
            ukeydf['open_sum'] = ukeydf['Open'] - ukeydf[f'ts_sum_{sum}']/10
            ukeydf['abs_close_vwap'] = np.abs(ukeydf['Close'] - ukeydf['vwap_5'])
        ...
        for _, ukeydf in group:
            print(i, end='\r')
            ukeydf.reset_index(drop=True, inplace=True)
            ukeydf['rank_open'] = ukeydf['open_sum'].rank(ascending=False)
            ukeydf['rank_abs'] = ukeydf['abs_close_vwap'].rank(ascending=False)
            df_list.append(ukeydf)
            i = i + 1
        data_0 = pd.concat(df_list, axis=0)
        data_0.eval(f"x = rank_open * -rank_abs", inplace=True)
        ...
        factor.to_csv('factor_alpha_12.csv.gz')
