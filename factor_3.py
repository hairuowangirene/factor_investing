...
class factor_3():
    ...
    def generate(self):
        """
        formula: SUM((CLOSE=DELAY(CLOSE,1)?0:CLOSE-(CLOSE>DELAY(CLOSE,1)?MIN(
        LOW,DELAY(CLOSE,1)):MAX(HIGH,D ELAY(CLOSE,1)))),6)
        ...
        for _, ukeydf in group:
            ...
            ukeydf = ts_delay(ukeydf, 'Close', n=delay)
            ukeydf['If_Close1'] = np.where(ukeydf['Close'] ==
                                           ukeydf[f'ts_delay_{delay}'], 0.00001,
                                           ukeydf['Close'])
            ukeydf['If_Close2'] = np.where(ukeydf['Close'] >
                                           ukeydf[f'ts_delay_{delay}'],
                                           np.minimum(ukeydf['Low'],
                                                      ukeydf[f'ts_delay_{delay}'
                                                      ]),
                                           np.maximum(ukeydf['High'],
                                                      ukeydf[f'ts_delay_{delay}'
                                                      ]))
            ukeydf['sum_total'] = ukeydf['If_Close1'] - ukeydf['If_Close2']
            ukeydf = ts_sum(ukeydf, 'sum_total', n=sum)
            df_list.append(ukeydf)
            i += 1
        data_0 = pd.concat(df_list, axis=0)
        data_0.eval(f"x = ts_sum_{sum}", inplace=True)
        ...
        factor.to_csv('factor_alpha_3.csv.gz')
