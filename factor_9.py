...
class factor_9():
    ...
    def generate(self):
        """
        formula: SMA(((HIGH+LOW)/2-(DELAY(HIGH,1)+DELAY(LOW,1))/2)*(
        HIGH-LOW)/VOLUME,7,2)
        ...
        data_0['High_Low'] = ((data_0["High"] + data_0['Low']) / 2)
        data_0['High_Low_Volume'] = (data_0['High'] - data_0['Low']) / \
                                    data_0['Volume']
        ...
        for _, ukeydf in group:
            ...
            ukeydf = ts_delay(ukeydf, 'High', n=delay)
            ukeydf.rename(
                columns={f'ts_delay_{delay}': f'ts_delay_high_{delay}'},
                inplace=True)
            ukeydf = ts_delay(ukeydf, 'Low', n=delay)
            ukeydf.rename(
                columns={f'ts_delay_{delay}': f'ts_delay_low_{delay}'},
                inplace=True)
            ukeydf['Ratio'] = ukeydf['High_Low'] - \
                              ((ukeydf[f'ts_delay_high_{delay}'] +
                                ukeydf[f'ts_delay_low_{delay}']) / 2) * \
                              ukeydf['High_Low_Volume']
            ukeydf = sma(ukeydf, "Ratio", n=sman)
            df_list.append(ukeydf)
            i = i + 1
        data_0 = pd.concat(df_list, axis=0)

        data_0.eval(f"x = sma_Ratio_{sman}2", inplace=True)
        ...
        factor.to_csv('factor_alpha_9.csv.gz')
