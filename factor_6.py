...
class factor_6():
    ...
    def generate(self):
        ...
        data_0['Open_High'] = (data_0['Open'] * 0.85 + data_0['High'])
        ...
        for _, ukeydf in group:
            ...
            ukeydf = ts_delta(ukeydf, 'Open_High', n=delta)
            ukeydf['ts_sign_delta'] = np.sign(ukeydf[f'ts_delta_{delta}'])
        ...
        for _, ukeydf in group:
            print(i, end='\r')
            ukeydf.reset_index(drop=True, inplace=True)
            ukeydf['rank_ts_delta'] = ukeydf['ts_sign_delta'].rank(
                ascending=False)
            df_list.append(ukeydf)
            i = i + 1
        data_0 = pd.concat(df_list, axis=0)
        ...
        factor.to_csv('factor_alpha_6.csv.gz')
