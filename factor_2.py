...
class factor_2():
    ...
    def generate(self):
        """
        formula: (-1 * DELTA((((CLOSE - LOW) - (HIGH - CLOSE)) / (HIGH-LOW)),
        1))
        ...
        data_0['Close_Low'] = data_0['Close'] - data_0['Low']
        data_0['High_Close'] = data_0['High'] - data_0['Close']
        data_0['High_Low'] = data_0['High'] - data_0['Low']
        data_0['Ratio'] = (data_0['Close_Low'] - data_0['High_Close']) / \
                          (data_0['High_Low'] + 0.00001)
        ...
        for _, ukeydf in group:
            ...
            ukeydf = ts_delta(ukeydf, columns='Ratio', n=delta)
            df_list.append(ukeydf)
            i += 1
        data_0 = pd.concat(df_list, axis=0)
        data_0.eval(f"x = -ts_delta_{delta}", inplace=True)
        ...
        factor.to_csv('factor_alpha_2.csv.gz')
