...
class factor_7():
    ...
    def generate(self):
        """
        formula: ((RANK(MAX((VWAP - CLOSE), 3)) + RANK(MIN((VWAP - CLOSE),
        3))) * RANK(DELTA(VOLUME, 3)))
        ...
        for _, ukeydf in group:
            ...
            ukeydf = vwap(ukeydf)
            ukeydf['vwap_close'] = ukeydf['vwap_5'] -ukeydf['Close']
            ukeydf = ts_max(ukeydf, 'vwap_close', n=max)
            ukeydf = ts_min(ukeydf, 'vwap_close', n=min)
            ukeydf = ts_delta(ukeydf, 'Volume', delta)
        ...
        for _, ukeydf in group:
            print(i, end='\r')
            ukeydf.reset_index(drop=True, inplace=True)
            ukeydf['rank_max'] = ukeydf[f'ts_max_{max}'].rank(ascending=False)
            ukeydf['rank_min'] = ukeydf[f'ts_min_{min}'].rank(ascending=False)
            ukeydf['rank_delta'] = ukeydf[f'ts_delta_{delta}']\
                .rank(ascending=False)
            df_list.append(ukeydf)
            i = i + 1
        data_0 = pd.concat(df_list, axis=0)
        data_0.sort_values(by=['ukey', 'DataDate'], inplace=True)
        data_0.eval(f"x = (rank_max + rank_min) * rank_delta", inplace=True)
        ...
        factor.to_csv('factor_alpha_7.csv.gz')
