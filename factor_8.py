...
class factor_8():
    ...
    def generate(self):
        """
        formula: RANK(DELTA(((((HIGH + LOW) / 2) * 0.2) +
         (VWAP * 0.8)), 4) * -1)
        @Description :
        @Params      :
        @Returns     :
        ...
        data_0['High_Low'] = ((data_0["High"] + data_0['Low']) / 2) * 0.2
        ...
        for _, ukeydf in group:
            ...
            ukeydf = vwap(ukeydf)
            ukeydf['High_Low_VWAP'] = ukeydf['High_Low'] + ukeydf['vwap_5'] * 0.8
            ukeydf = ts_delta(ukeydf, 'High_Low_VWAP', n = delta)
        ...
        for _, ukeydf in group:
            print(i, end='\r')
            ukeydf.reset_index(drop=True, inplace=True)
            ukeydf['rank_ts_delta'] = ukeydf[f'ts_delta_{delta}'].\
                rank(ascending=False)
            df_list.append(ukeydf)
            i = i + 1
        data_0 = pd.concat(df_list, axis=0)
        data_0.sort_values(by=['ukey', 'DataDate'], inplace=True)
        data_0.eval(f"x = - rank_ts_delta", inplace=True)
        ...
        factor.to_csv('factor_alpha_8.csv.gz')
